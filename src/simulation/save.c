...
void sim_save_current_state(verlet_sim_t *sim, char * file_path){
    ...
    fprintf(file_out, "%zu, %u, %u, %zu, %u, %u, %u, %d\n", sim_get_object_count(sim), sim_get_width(sim), sim_get_height(sim), sim_get_current_step(sim),
                                                                    sim_get_constraint_x(sim), sim_get_constraint_y(sim), sim_get_constraint_radius(sim), sim_get_shape(sim));
...
verlet_sim_t * sim_load_file(char * file_path){
    ...
    unsigned int width, height, constraint_x, constraint_y, constraint_radius;
    int shape;
    int matches = fscanf(save_file, "%zu, %u, %u, %zu, %u, %u, %u, %d\n", &circle_count, &width, &height, &steps, &constraint_x, &constraint_y, &constraint_radius, &shape);
    if (matches != 10){
    ...
    verlet_sim_t * s = init_simulation(constraint_shape, constraint_x, constraint_y, constraint_radius, width, height, 0, 1000);
    assert(s);
